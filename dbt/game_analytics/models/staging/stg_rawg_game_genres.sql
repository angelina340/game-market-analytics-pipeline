with source_data as (
    select
        extracted_at,
        payload
    from {{ source('raw', 'RAWG_GAMES_RAW') }}
),

latest_extract as (
    select max(extracted_at) as extracted_at
    from source_data
),

flattened_games as (
    select
        source_data.extracted_at,
        game.value as game_json
    from source_data
    inner join latest_extract
        on source_data.extracted_at = latest_extract.extracted_at,
    lateral flatten(input => payload:payload:results) as game
),

flattened_genres as (
    select
        game_json:id::number as rawg_game_id,
        game_json:name::string as game_name,
        extracted_at,
        genre.value as genre_json
    from flattened_games,
    lateral flatten(input => game_json:genres) as genre
)

select
    rawg_game_id,
    game_name,
    genre_json:id::number as genre_id,
    genre_json:name::string as genre_name,
    genre_json:slug::string as genre_slug,
    extracted_at as extracted_at_utc
from flattened_genres
qualify row_number() over (
    partition by rawg_game_id, genre_json:id::number
    order by extracted_at desc
) = 1
